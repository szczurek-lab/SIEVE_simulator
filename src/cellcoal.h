...
#include "statisticalTools.h"       // added code
...
static void SimulateISMforSite(TreeNode *p, int genome, int site,
                               int doISMhaploid, long int *seed);
...
// added code
static void FillSubstitutionMatrixExtd(double **EPij, double branchLength,
                                       int nrOfGenotypes);
static void MuDelModel(double **EPij, double branchLength);
static void MuInDelModel(double **EPij, double branchLength);
// end added code
...
static double *periodGrowth, growthRate, sequencingError, ADOrate,
    ...
    meanAmplificationError, varAmplificationError, doubletRate;
